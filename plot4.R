...
############################################################################################################
# Before rebuilding the data set, I check to see if the data set already exists.  If I built it 
# previously I saved it using: saveRDS(ds1, file="household_power_consumption.rds")
#
...
# if data file doesn't exist, build it:

if(!file.exists("household_power_consumption.rds")){
#############################################################################################################
###  Using modified method from: http://www.ocf.berkeley.edu/~mikeck/?p=688 to download, unzip, and build data set
### Uses temporary file and temporary directory to store the downloaded and extracted files
#############################################################################################################
...
# The data is downloaded to a temporary file, and the full path is contained in tf.
# This also creates a temporary directory for extracting the
# data. The unzip() function is used to query the contents of a zip file
# and extract files to a specified location. Am assuming the zip file will always contain a single file
# in text format.
...
unlink(tf)
unlink(td)
# Set dates to keep (these are in the form of day/month/Year)
